...
def draw_colored_boxplot(ax, values, position, color, width):
    ...
    # 3) box outline on top so it is not cut by median
    ax.plot(
        [x0, x1, x1, x0, x0],
        [q1, q1, q3, q3, q1],
        color=light_color,
...
#%% omissions in first 20 trials after switch


def process_session_omission_counts(row, window=20):
    try:
        df = read_csv_cached(row["path"]).copy()
    except Exception as e:
        print(f"⚠ Could not read {row['filename']}: {e}")
    ...
    for i in range(1, len(df)):
        ...
        next_trials = df.iloc[i:i + window]

        # 🔥 key difference: count omissions directly
        omissions = (next_trials["omission"] == 1).sum()
...
def compute_omission_after_switch_tables(sessions_df, window=20):
    print("\n=== Computing omissions after switch ===")

    records = []

    for _, row in sessions_df.iterrows():
        res = process_session_omission_counts(row, window=window)
        if res is None:
            continue
...
def plot_omissions_triangle_animals(om_df):
    df_long = om_df.melt(
        id_vars=["animal", "session_date", "session_id", "strain", "stage"],
        value_vars=["Action→Sound", "Sound→Action"],
        var_name="switch_type",
        value_name="omission_count"
    )

    return plot_animal_stage_metric_by_category(
        df=df_long,
        category_col="switch_type",
        value_col="omission_count",
        category_order=["Action→Sound", "Sound→Action"],
        category_labels=["Action→Sound", "Sound→Action"],
        ylabel="Number of omissions",
        title="Omissions after block switches (first 20 trials)",
        strain_colors=STRAIN_COLORS,
        selected_strains=SELECTED_STRAINS,
...
#%% Time course of omissions

def compute_omission_timecourse_tables(sessions_df, window=20):
    print("\n=== Computing omission time course after switch ===")
    ...
    for _, row in sessions_df.iterrows():
        ...
        needed_cols = {"block", "omission"}
        if not needed_cols.issubset(df.columns):
            print(f"⚠ Missing columns for omission time course in {row['filename']}")
            continue
        ...
        for i in range(1, len(df)):
            ...
            for k, (_, tr) in enumerate(next_trials.iterrows(), start=1):
                omission = tr["omission"] if not pd.isna(tr["omission"]) else np.nan
    ...
    if df_out.empty:
        print("⚠ No omission time course data computed.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    return df_out, session_timecourse_df


def plot_omission_timecourse(session_timecourse_df, stage, strain_colors):
    df = session_timecourse_df.copy()
    df = filter_selected_strains(df, SELECTED_STRAINS)
    df = df[df["stage"] == stage].copy()

    if df.empty:
        print(f"⚠ No omission time course data for stage: {stage}")
        return None, None
    ...
    for ax, switch_type in zip(axes, switch_order):
        df_sw = df[df["switch_type"] == switch_type]

        for strain in present_strains:
            d_strain = df_sw[df_sw["strain"] == strain]
            if d_strain.empty:
                continue

            stats_df = (
                d_strain.groupby("trial_from_switch")["omission"]
                .agg(["mean", "sem"])
                .reset_index()
                .sort_values("trial_from_switch")
            )
            ...
            ax.plot(x, y, linewidth=2.8, color=strain_colors[strain])
            ax.fill_between(x, y - sem, y + sem,
                            color=strain_colors[strain], alpha=0.18)

        ax.set_xlim(1, int(df_sw["trial_from_switch"].max()))
        ax.set_ylim(0, 100)
    ...
    axes[0].set_ylabel("Omissions (%)", fontsize=plot_config["label_fontsize"])

    fig.suptitle(
        f"Omission Time Course After Switch — {STAGE_LABELS[stage]}",
        fontsize=plot_config["title_fontsize"],
        y=1.02
    )
...
def main():
    ...
    fig, _ = plot_omissions_triangle_animals(om_switch_df)
    if fig is not None:
        save_figure(fig, "omissions_switchtype_per_animal_stage")
        
    fig, _ = plot_omission_timecourse(session_om_tc_df, stage="naive", strain_colors=STRAIN_COLORS)
    if fig is not None:
        save_figure(fig, "omission_timecourse_pre")
