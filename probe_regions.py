...
OUTPUT_FOLDER_NAME = "probe_region_profiles_barstyle"
...
def load_probe_csv(csv_path: Path, use_only_inside_brain: bool = True) -> pd.DataFrame:
    ...
    if depth_col is None:
        raise ValueError(f"{csv_path.name} is missing depth(um). Available columns:\n{list(df.columns)}")
...
PROBE_FOLDER = Path(r"L:/dmclab/Joana/PFC-Str_behavior_project/Histology/986170/all_probes")
FILE_PATTERN = "*neuropixels_probe*.csv"

OUTPUT_DIR = Path(r"L:/dmclab/Joana/PFC-Str_behavior_project/Histology/986170/all_probes/probe_region_profiles_barstyle")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
